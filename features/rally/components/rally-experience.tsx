...
import { Camera, CheckCircle2, Crown, Flame, Loader2, Send, Trophy, UserRound, X } from "lucide-react";
...
/**
 * The full member experience — streak hero, day grid, check-in, feeds,
 * peer voting, standings. Used by the public room (identity picked on-device)
 * and the host room (identity fixed to the crowned seat, admin controls on).
 */
export function RallyExperience({
  ...
  view: RallyView;
  /** Lock identity to this member (host room). Omit for on-device picker (public room). */
  fixedMemberId?: string | null;
  /** Show host approve/reject override on check-ins. */
            ...
            <Card className={cn("space-y-4", me.checkedInToday ? "border-success/40 shadow-glow" : "bg-gold-tint shadow-red-glow")}>
              ...
              {me.checkedInToday ? (
                ...
                >
                  ...
                  {justCheckedIn ? `Day ${view.dayNumber} locked in! 🔥 The streak lives.` : "Checked in for today. See you tomorrow!"}
                </motion.p>
              ) : canCheckInToday ? (
